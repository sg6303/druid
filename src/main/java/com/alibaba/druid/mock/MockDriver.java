...
public class MockDriver implements Driver, MockDriverMBean {
	...
	public ResultSet executeQuery(MockStatementBase stmt, String sql)
			throws SQLException {
	...
	public ResultSet createResultSet(MockPreparedStatement stmt) {
		...
		if ("SELECT 1".equalsIgnoreCase(sql)) {
			...
			rs.getRows().add(
					new Object[] { new java.sql.Timestamp(System
							.currentTimeMillis()) });
	...
	public MockConnection createMockConnection(MockDriver driver, String url,
			Properties connectProperties) {
		return new MockConnection(this, url, connectProperties);
	}

	public MockPreparedStatement createMockPreparedStatement(
			MockConnection conn, String sql) {
	...
	public MockCallableStatement createMockCallableStatement(
			MockConnection conn, String sql) {
